...
public class ShoppingConsultRemoveTest {
	...
	@Test
	public void ConsultRemoveTest () throws IOException {
		...
		login.log.info("Le User est bien connecté");
		test.log(LogStatus.PASS, "Connexion réussie");
		...
		test.log(LogStatus.PASS, "Consultation de mes commandes (My orders)");
		
		//Suppression de commande
		login.log.info("Suppression d’une commande existante dans votre compte ( remove)");
		driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
		shopping.getRemove_btn().click();
		driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
		shopping.getCancel_btn().click();
		login.log.info("1 article a été supprimé");
		test.log(LogStatus.PASS, "Commande supprimée");
